...
KTD4. **Ready Player Me (RPM) avatar with robotic skin, using @readyplayerme/visage.** This is the most technically reliable path for browser-based lip-sync with convincing quality. RPM avatars embed 52 ARKit-compatible viseme morph targets; the `@readyplayerme/visage` library (React Three Fiber + Three.js) drives them from the browser. A heavily stylized robotic/metallic avatar (no organic hair, glowing eyes, geometric proportions) satisfies the "not human or anime" aesthetic while using the best-supported lip-sync infrastructure available. The avatar glTF can be hosted on R2. (session-settled: user-approved — user deferred to agent recommendation; chosen over custom non-humanoid geometry because the RPM viseme infrastructure is production-proven in AI assistant applications)

KTD5. **Lip-sync driven by browser-side Web Audio API viseme analysis.** DeepInfra's TTS API returns audio only — no phoneme or timing data. The browser analyzes the audio stream in real time using `AnalyserNode` FFT frequency data to infer phoneme-approximate viseme weights, which drive the RPM avatar's ARKit blend shapes during playback. This produces convincing-approximation sync without external phoneme analysis services. (session-settled: user-directed — chosen over amplitude-only jaw-open; viseme approximation is more convincing for a showcase)
...
- An RPM avatar glTF with robotic/metallic styling is obtainable via a free RPM developer account. The file is downloaded once, hosted in R2, and served via CDN.
...
- `@readyplayerme/visage` — MIT-licensed, npm-installable, built on react-three-fiber + three.js + drei; handles glTF loading, ARKit viseme morph targets, animation. Source: [github.com/readyplayerme/visage](https://github.com/readyplayerme/visage)
